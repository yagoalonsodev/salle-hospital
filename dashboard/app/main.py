...
def render_pipeline(runs: list) -> None:
    st.subheader("Estado del pipeline")
    if not runs:
        st.info("Sin ejecuciones registradas en `pipeline_runs`.")
        return
    ...
    c1.metric("OK", ok)
    c2.metric("En curso", running)
    c3.metric("Fallidos", failed)


def render_quality(quality: dict) -> None:
    st.subheader("Calidad de datos y errores")
    by_type = quality.get("by_type") or []
    if by_type:
        df = pd.DataFrame(by_type)
        fig = px.bar(df, x="issue_type", y="count", title="Incidencias por tipo")
        st.plotly_chart(fig, use_container_width=True)
    st.metric("Total incidencias", quality.get("total", 0))
...
tab_ia, tab_img, tab_pipe, tab_alert = st.tabs(
    ["IA y clases", "Imágenes", "Pipeline y calidad", "Alertas"]
)
...
with tab_pipe:
    render_pipeline(data.get("pipeline_runs") or [])
    st.divider()
    render_quality(data.get("quality") or {})
